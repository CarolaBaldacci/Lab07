/**
 * Sample Skeleton for 'Scene.fxml' Controller Class
 */

package it.polito.tdp.poweroutages;

import java.net.URL;
import java.util.List;
import java.util.ResourceBundle;
import it.polito.tdp.poweroutages.model.Model;
import it.polito.tdp.poweroutages.model.Nerc;
import it.polito.tdp.poweroutages.model.PowerOutages;
import javafx.event.ActionEvent;
import javafx.fxml.FXML;
import javafx.scene.control.ComboBox;
import javafx.scene.control.TextArea;
import javafx.scene.control.TextField;

public class FXMLController {

	//public List <Nerc> nercList ; 
	public Model model;
	
	@FXML // ResourceBundle that was given to the FXMLLoader
    private ResourceBundle resources;

    @FXML // URL location of the FXML file that was given to the FXMLLoader
    private URL location;

    @FXML // fx:id="cmbNerc"
    private ComboBox<Nerc> cmbNerc; // Value injected by FXMLLoader

    @FXML // fx:id="txtYears"
    private TextField txtYears; // Value injected by FXMLLoader

    @FXML // fx:id="txtHours"
    private TextField txtHours; // Value injected by FXMLLoader

    @FXML // fx:id="txtResult"
    private TextArea txtResult; // Value injected by FXMLLoader
    
    @FXML
    void doRun(ActionEvent event) {
    	txtResult.clear();
    	//Controlli di input
    	Nerc nercSelected= cmbNerc.getValue();
    	if(nercSelected == null) {
    		txtResult.appendText("Selezionare un nerc");
    		return ;
    	}
    	int maxY= Integer.parseInt(txtYears.getText());
    	if(maxY<=0) {
    		txtResult.appendText("Fornire un numero massimo di anni");
    		return ;
    	}
    	int maxH= Integer.parseInt(txtHours.getText());
    	if(maxH<=0) {
    		txtResult.appendText("Fornire un numero massimo di ore");
    		return ;
    	}
    	
    	//Esecuzione
    	List<PowerOutages> worstCase=model.getWorstCase(maxH, maxY, nercSelected);
    	
    	//Stampa
    	txtResult.setText(String.format("Worst case analisys..  %d max hours and %d max years", maxH, maxY));
    	for(PowerOutages po: worstCase) {
    		txtResult.appendText(po.toString());
    		txtResult.appendText("\n");
    	}
    	
    	
    }

    @FXML // This method is called by the FXMLLoader when initialization is complete
    void initialize() {
        assert cmbNerc != null : "fx:id=\"cmbNerc\" was not injected: check your FXML file 'Scene.fxml'.";
        assert txtYears != null : "fx:id=\"txtYears\" was not injected: check your FXML file 'Scene.fxml'.";
        assert txtHours != null : "fx:id=\"txtHours\" was not injected: check your FXML file 'Scene.fxml'.";
        assert txtResult != null : "fx:id=\"txtResult\" was not injected: check your FXML file 'Scene.fxml'.";
        
        // Utilizzare questo font per incolonnare correttamente i dati;
        txtResult.setStyle("-fx-font-family: monospace");
    }
    
    /*private void setCmbItems() {
    	nerc = model.getNercList();
    	cmbNerc.getItems().addAll(nerc);
    }  */
    
    public void setModel(Model model) {
    	this.model = model;
    	//setCmbItems();
    	List <Nerc> nercList = model.getNercList();
    	cmbNerc.getItems().addAll(nercList);
    }
}
