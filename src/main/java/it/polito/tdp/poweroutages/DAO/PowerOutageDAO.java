...
public class PowerOutageDAO {
	...
	public List<PowerOutages> getPowerOutagesByNerc(Nerc nerc) {
		...
		String sql = "SELECT id, nerc_id, date_event_began, date_event_finished, customers_affected FROM poweroutages WHERE nerc_id=?";
		...
		try {
			...
			while (res.next()) {
				PowerOutages p= new PowerOutages(res.getInt("id"), nerc, res.getTimestamp("date_event_began").toLocalDateTime(),
